...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (computedBounds.mode === "all") {
        ...
        expRes = await expQ.gte("expense_date", computedBounds.start as string).order("expense_date", { ascending: false });
      }
  ...
  const minMax = useMemo(() => {
    if (expenses.length === 0) return { min: null as string | null, max: null as string | null };
    let min = expenses[0].expense_date;
    let max = expenses[0].expense_date;
    for (const e of expenses) {
      if (e.expense_date < min) min = e.expense_date;
      if (e.expense_date > max) max = e.expense_date;
    }
    return { min, max };
  }, [expenses]);
  ...
  const daysInRange = useMemo(() => {
    ...
    if (computedBounds.mode === "between" && computedBounds.start && computedBounds.endExclusive) {
    ...
    if (computedBounds.mode === "gte") {
      const n = Number(range);
      return Number.isFinite(n) && n > 0 ? n : 1;
    }
    if (computedBounds.mode === "all" && minMax.min && minMax.max) {
      const s = new Date(`${minMax.min}T00:00:00`).getTime();
      const e = new Date(`${minMax.max}T00:00:00`).getTime();
      return Math.max(1, Math.floor((e - s) / (24 * 60 * 60 * 1000)) + 1);
    }
    return 1;
  }, [computedBounds, range, minMax]);
  ...
  const maxCat = useMemo(() => byCategoryAll.reduce((m, x) => Math.max(m, x.total), 0), [byCategoryAll]);
  ...
  const trend = useMemo(() => {
    ...
    // For "All time", show only last 31 days to avoid huge loops.
    const start =
      computedBounds.mode === "all"
        ? isoDaysAgo(30)
        : (computedBounds.start as string);

    let endInclusive: string;

    if (computedBounds.mode === "eq") {
      endInclusive = computedBounds.start as string;
            ...
            {trendOpen ? (
              <div style={{ padding: "0 12px 12px 12px" }}>
                <div style={{ fontSize: 12, opacity: 0.7, marginTop: 2 }}>
                  {computedBounds.mode === "all"
                    ? "Showing last 31 days • 0-spend days included"
